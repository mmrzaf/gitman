...
func TestEmbeddedTemplatesExecuteWithRepresentativeTypedPages(t *testing.T) {
	...
	commitDetail := git.CommitDetail{Hash: commit.Hash, RefsTruncated: true, Parents: []string{"fedcba9876543210fedcba9876543210fedcba98"}, Author: commit.Author, Email: commit.Email, Date: now, Subject: commit.Message, Body: "A deliberately small change.", Branches: []string{"main"}}
	diff := git.CommitDiff{TotalFiles: 1, Additions: 1, Deletions: 1, FilesTruncated: true, StatsTruncated: true, Files: []git.FileDiff{{ChangedFile: git.ChangedFile{Status: "modified", StatusCode: "M", Path: "main.go", Additions: 1, Deletions: 1}, Hunks: []git.DiffHunk{{Header: "@@ -1 +1 @@", Lines: []git.DiffLine{{Kind: "delete", OldLine: 1, Text: "old"}, {Kind: "add", NewLine: 1, Text: "new"}}}}}}}
	...
	cases := map[string]pageModel{
		...
		"tokens.html":             &TokensPageData{PageData: base("Tokens", nil), Tokens: []AccessTokenView{{AccessToken: models.AccessToken{ID: "t1", Name: "Laptop", CreatedAt: now}, State: "Active", StateClass: "success", ScopeLabel: "Read-only"}}, NewToken: "gm_example"},
		"security.html":           &SecurityPageData{PageData: base("Security", nil), Events: []SecurityEventView{{Summary: "Signed in", SourceIP: "192.0.2.1", CreatedAt: now}}},
		"repo_view.html":          &RepoPageData{PageData: base("Files", nav("files", "", "main")), Owner: owner, Repository: repo, CurrentRef: "main", CurrentRefKind: "branch", ResolvedCommit: commit.Hash, Branches: []string{"main", "develop"}, Tags: []string{"v1.0.0"}, RefsTruncated: true, Tree: []git.TreeEntry{{Type: "tree", Name: "internal"}, {Type: "blob", Name: "README.md", Size: 1234}}, LatestCommit: &commit, LatestCI: &run, ReadmePath: "README.md", ReadmeContent: "# Project\n\nSmall and boring.", TreeTruncated: true},
